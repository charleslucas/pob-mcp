...
| `analyze_item_mods` | Identify each mod line on an existing item and report its tier + next-tier upgrade. Pass `mod_lines` (the item's explicit prefix/suffix text, one per array entry) plus optional `base_name` and `ilvl`. For each line: matched mod ID, affix name, mod group, tier rank (e.g. T3 of 13), and the next tier's required ilvl + value range. Matches by stripping the rolled value and finding the tier whose range contains it; prefers affixed/naturally-rollable mods over Hellscape/influence range-overlaps. `{crafted}` lines are matched against the bench-craft pool (ModMaster.lua); `{fractured}` against the natural pool; `{enchanted}` reported but not indexed. Collapses hybrid-mod continuation lines. |
